    ...
    <div class="w-full h-[88%] bg-gray-200 flex items-center justify-center">
        <div class="w-[90%] h-5/6 bg-white rounded-xl pt-3 flex flex-col items-center">
            ...
            <div class="flex w-full h-4/5">
                <form action="{{route('products.update',compact('product'))}}" method="post" class="w-full h-full flex flex-row-reverse">
                    @csrf
                    <div class="w-1/2 h-full flex flex-col items-end pr-20 relative">
                        ...
                        <div class="w-5/6 h-auto flex flex-row-reverse justify-between pt-4 mb-6">
                            ...
                            <textarea name="description" id="description" cols="5" rows="5" class="w-2/5 rounded outline-0 p-2 border border-gray-400">{{$product->description}}</textarea>
                            @error('description')
                            <p class="text-red-700">{{$message}}</p>
                            @enderror
                        </div>
                    ...
                    <div class="w-1/2 h-full flex flex-col items-end pr-20">
                        <div class="w-5/6 h-auto flex flex-row-reverse justify-between pt-4 mb-6">
                            ...
                            <input type="number" name="price" step="100000" min="0" value="{{$product->price}}" id="price" class="w-2/5 h-8 rounded outline-0 p-2 border border-gray-400">
                            @error('price')
                            <p class="text-red-700">{{$message}}</p>
                            @enderror
                        </div>
